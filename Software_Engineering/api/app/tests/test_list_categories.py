...
def test_list_categories_trends_valid(client):
    
    response = client.post("/api/categories/trends")
    assert response.status_code == 200
    response_json = response.json()
    assert isinstance(response_json, list)
    

def test_list_categories_trends_valid_with_params(client):
    ...
    response = client.post(f"/api/categories/trends?{query_string}", json=test_params)
    assert response.status_code == 200
    response_json = response.json()
    assert isinstance(response_json, list)