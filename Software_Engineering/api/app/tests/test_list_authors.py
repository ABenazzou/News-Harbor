...
def test_list_authors_frequency_valid(client):
    
    response = client.post("/api/authors/frequency")
    assert response.status_code == 200
    response_json = response.json()
    assert isinstance(response_json, list)
    

def test_list_authors_frequency_valid_with_params(client):
    ...
    response = client.post(f"/api/authors/frequency?{query_string}", json=test_params)
    assert response.status_code == 200
    response_json = response.json()
    assert isinstance(response_json, list)
