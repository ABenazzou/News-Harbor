...
const Harbor = () => {
  ...
  const formGroups = [
    ...
    {
      controlId: "subcategories",
      label: "Subcategories",
      options: subcategoryOptions,
      setter: setFilterSubcategories,
      ref: subcategorySelectRef,
    },
    {
      controlId: "topicsSelect",
      label: "Topics",
      options: topicOptions,
      setter: setFilterTopics,
      ref: topicSelectRef,
    },
    {
      controlId: "authorsSelect",
      label: "Authors",
      options: authorOptions,
      setter: setFilterAuthors,
      ref: authorSelectRef,
    },
  ];
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/articles?limit=${itemsPerPage}&offset=${offset}&${filterQuery}`,
      options
    )
  ...
  useEffect(() => {
    ...
    const authorsQuery = filterAuthors
      .map((author) => `authors=${author}`)
      .join("&");
    const subcategoriesQuery = filterSubcategories
      .map((subcategory) => `subcategories=${subcategory}`)
      .join("&");
    const topicsQuery = filterTopics
      .map((topic) => `topics=${topic}`)
      .join("&");

    const categoriesFilterQuery = [
      authorsQuery,
      subcategoriesQuery,
      topicsQuery,
    ].filter((query) => query !== "");
  ...
  useEffect(() => {
    ...
    const authorsQuery = filterAuthors
      .map((author) => `authors=${author}`)
      .join("&");
    const categoriesQuery = filterCategories
  ...
  useEffect(() => {
    ...
    const subCategoriesFilterQuery = [
      authorsQuery,
      categoriesQuery,
      topicsQuery,
    ].filter((query) => query !== "");
  ...
  useEffect(() => {
    setAuthorOptions(
      authors.map((item) => ({ value: encodeURIComponent(item), label: item }))
    );
  }, [authors]);
  ...
  useEffect(() => {
    setTopicOptions(
      topics.map((item) => ({ value: encodeURIComponent(item), label: item }))
    );
  }, [topics]);
          ...
          <Col md={2} key={idx}>
            <Form.Group controlId={group.controlId} key={group.controlId}>
              ...
              <Select
                ...
                onChange={(selected) =>
                  group.setter(selected.map((option) => option.value))
                }
                styles={{
                  control: (base) => ({
                    ...base,
