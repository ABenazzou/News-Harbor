      ...
      .then((data) => {
        if (data.images) {
          const filteredImages = data.images.filter(
            (image) =>
              !image.includes("presentational_grey_line") &&
              !image.includes("_line")
          );
    ...
    <Container className="article-body">
      <Row>
        <Col>
          ...
          {article.subtitle && (
            <h4 className="mb-3 muted">{article.subtitle}</h4>
          )}

          <div className="mb-4">
            {article.date_posted && (
              <Badge bg="secondary" className="me-2">
                Posted on: {formatDate(article.date_posted)}
              </Badge>
            )}
            {article.category && (
              <Badge bg="info">Category: {article.category}</Badge>
            )}
            {article.subcategory && (
